...
## Current File Structure (Vite + React — v0.15.0)
...
Archetype color is registered in `src/constants/archetypes.js` (`ARCH_COLORS`). Archetypes in `matchupWarnings.js` rules that are absent from `ARCH_COLORS` will render a badge with no color (badge shape still shows). These are marked ⚠ below — add to `ARCH_COLORS` if any fighter in `fighter-seed.json` uses them.
...
| MUAY THAI | `--orange` (suggested) | — | ⚠ used in matchupWarnings rules, not yet in ARCH_COLORS |
| CLINCH FIGHTER | `--purple` (suggested) | — | ⚠ used in matchupWarnings rules, not yet in ARCH_COLORS |
...
### Current State (Vite + React — v0.15.0)
