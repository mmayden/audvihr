...
- **What to test:**
  ...
  - Screens and tabs are integration-heavy — smoke tests only (renders without crashing). Full rendering tests deferred to Phase 8+.
...
│   ├── fetch-data.js         UFCStats scraper (Node ESM, cheerio, native fetch)
...
- **Fighter and event data is live (build-time scraped).** `fighters.js` and `events.js` are generated by `scripts/fetch-data.js` at `npm run build`. Do not hand-edit them. Markets and news remain mock until Phase 7+.
- **No live API calls from the client yet.** Phase 7 will introduce The Odds API via `useOdds` hook. Until then, all runtime data is static from the build.
