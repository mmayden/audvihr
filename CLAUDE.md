...
| `recent_fighters` | sessionStorage | `FighterScreen` | last 3 viewed fighter IDs (Phase 14) |
| `pick_log` | localStorage | `src/utils/pickLog.js` | pre-fight picks + outcomes (200-entry cap) (Phase 14) |
...
- **`pick_log` localStorage key is owned exclusively by `src/utils/pickLog.js`.** No other module reads or writes it. Reads must be `try/catch` with typed default `[]`. All stored values are plain text strings — no HTML, no eval, no template-literal markup in stored data.
...
│   └── statTiers.js          STAT_TIERS — per-stat thresholds for ELITE/ABOVE AVG/AVG/BELOW AVG labels (Phase 14)
├── data/
...
│   ├── fighters.js           findFighterByName(name, fighters) — pure name → ID lookup (Phase 14)
│   ├── percentiles.js        computePercentiles(fighter, allFighters) — per-stat percentile rank vs roster (Phase 14)
│   └── pickLog.js            readPickLog(), appendPick(), updatePickOutcome() — owns pick_log key (Phase 14)
├── components/
...
│   ├── FighterSearch.jsx     Type-to-search input with filtered dropdown (Phase 14)
