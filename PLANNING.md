...
## Current File Structure (Vite + React — v0.12.x)
...
### Current State (Vite + React — v0.12.0)
...
**Upcoming surfaces (Phase 13):**
- **React Router** — URL state for shareable links. Rules: no secrets in URL params, no API keys, no localStorage tokens. Only fighter IDs and screen slugs. Validate URL params as integers before FIGHTERS lookup. History API navigation does not require new CSP entries.
- **Export (markdown/CSV)** — client-side Blob download only. No data transmission to external services. CSV values must guard against formula injection (prefix `=`, `+`, `-`, `@` with `'`). If a third-party library is introduced (`jspdf`, `html2canvas`), run `npm audit` before merge and document accepted findings.
