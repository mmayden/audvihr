...
│   └── clv.js                appendCLVEntries(), readCLVLog() — CLV log localStorage helpers
├── components/
...
│   ├── CompareScreen.jsx
│   ├── CalendarScreen.jsx
│   ├── MarketsScreen.jsx     Unified live market dashboard (sportsbook + Polymarket + Kalshi)
...
- **CLV log in localStorage.** `src/utils/clv.js` owns the CLV log key and cap (500 entries). Do not write to `clv_log` from any other path.
