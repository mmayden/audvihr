...
- **New CSS tokens must be declared in all three theme blocks.** Any new CSS variable added to `:root` that has a theme-relevant value must also be declared in `[data-theme="light"]` and the `@media (prefers-color-scheme: light)` block. Missing declarations cause invisible/fallback rendering in light mode.
...
- **Current test count: 481 passing.** Do not merge changes that reduce this number without a documented reason.
- **Current version: v0.18.0.** Update `package.json` `version`, `MenuScreen.jsx` version badge, and `CHANGELOG.md` before merging any new phase to `master`.
